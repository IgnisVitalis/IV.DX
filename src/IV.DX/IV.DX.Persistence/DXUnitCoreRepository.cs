...
namespace IV.DX.Persistence
{
    internal partial class DXCoreRepository : IDXUnitCoreRepository, IDXStructureRepository, IDXEnumCoreRepository, IDXStructureRawReader, IDXElementCoreRepository, IDXRawReader
    {
        ...
        private RowItem BuildRowItemFromRow(DataRow row, DXMainTableDefinition structure)
        {
            ...
            var columns = structure.GetColumns();
        ...
        private static void SetDerivedDXUnitTypeIfPresent(DataTable? table, Guid recordId, Guid derivedTypeId)
        {
            if (table == null || DXMaintenanceToken.IsCoreInitializing)
                return;

            var column = FindColumn(table, Constants.DerivedDXUnitType);
            if (column == null || column.ReadOnly)
                return;

            var row = table.Rows.Find(recordId);
            if (row != null)
                row[column] = derivedTypeId;
        }
