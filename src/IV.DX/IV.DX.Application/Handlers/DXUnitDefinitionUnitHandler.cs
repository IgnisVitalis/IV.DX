...
namespace IV.DX.Application.Handlers
{
    ...
    {
        private readonly IDXUnitDataReader dxUnitDataReader;
        private readonly IDXUnitDataService dxUnitService;
        private readonly IDXUnitGenericRepository genericRepo;
        private readonly IDXStructureRepository dataStructureRepo;
        private readonly IDXElementGenericRepository dxElementGenericRepo;
        private readonly IDXStructureCache dxStructureCache;

        public DXUnitDefinitionUnitHandler(
        ...
        {
            this.dxUnitDataReader = dxUnitDataReader;
            this.dxUnitService = dxUnitService;
            this.genericRepo = genericRepo;
            this.dataStructureRepo = dataStructureRepo;
            this.dxElementGenericRepo = dxElementGenericRepo;
        ...
        public async Task<DXResult<DXUnitDefinitionUnit>> BeforeInsertAsync(DXUnitDefinitionUnit dxUnit, DXHandlerBaseContext ctx, CancellationToken ct)
        {
            ...
            if (ctx is DXUnitHandlerPreInitCoreContext)
            {
                ...
                await this.ProcessUniqueColumnsAsync(dxUnit, null, ct);
        ...
        public async Task<DXResult> AfterInsertAsync(DXUnitDefinitionUnit dxUnit, DXHandlerBaseContext ctx, CancellationToken ct)
        {
            ...
            else
            {
                ...
                await base.ProcessUniqueColumnsAsync(originalIncomeDXUnit!, null, ct);

                await ProcessBaseDXUnitAsync(dxUnit);
            }
        ...
        private async Task ProcessBaseDXUnitAsync(DXUnitDefinitionUnit dxUnit)
        {
            if (dxUnit.BaseDXUnit.HasValue)
            {
                var baseDXUnit = await dxUnitDataReader.GetItemAsync<DXUnitDefinitionUnit>(dxUnit.BaseDXUnit.Value);
                var existingRelation = baseDXUnit!.DXUnitToUnitRelationElement.Announced.SingleOrDefault(x => x.TargetRelationName == Constants.DerivedDXUnitType);

                if (existingRelation == null)
                {
                    baseDXUnit.DXUnitToUnitRelationElement.AddToAnnounced(this.GetRelationToDerivedDXUnitType(baseDXUnit));
                    await dxUnitService.UpdateAsync(baseDXUnit);
                }
            }
        }
        ...
        public async Task<DXResult<DXUnitDefinitionUnit>> BeforeUpdateAsync(DXUnitDefinitionUnit dxUnit, DXHandlerBaseContext ctx, CancellationToken ct)
        {
            ...
            await base.ProcessUniqueColumnsAsync(dxUnit, objectInfoFromDB, ct);
        ...
        private Task DeleteDXUnitToUnitRelationElements(DXUnitDefinitionUnit existingDXUnit, DXHandlerBaseContext ctx, CancellationToken ct)
        {
        ...
        private Task DeleteDXUnitToElementRelationElements(DXUnitDefinitionUnit existingDXUnit, DXHandlerBaseContext ctx, CancellationToken ct)
        {
        ...
        {
            var targetUnit = genericRepo.GetDXUnit<DXUnitDefinitionUnit>(dxUnitToUnitRelationElement.TargetDXUnit);
            if (targetUnit?.DXUnitToUnitRelationElement?.Announced.Any(x =>
                    x.TargetDXUnit == dxUnitToUnitRelationElement.DXUnitID
                    && x.OwnRelationName == dxUnitToUnitRelationElement.TargetRelationName
                    && x.TargetRelationName == dxUnitToUnitRelationElement.OwnRelationName) == true)
                return;
        ...
        {
            if (relatedDXUnit?.DXUnitToUnitRelationElement == null)
                return;

            var revertedElementsToDelete = relatedDXUnit.DXUnitToUnitRelationElement.Announced
                .Where(x => x.TargetDXUnit == DXUnitToUnitRelationElement.DXUnitID)
                .ToList();

            foreach (var element in revertedElementsToDelete)
                dxElementGenericRepo.Delete(element);
        }
        ...
        private void AssignDXUnit(DXUnitDefinitionUnit dxUnit, DXUnitToUnitRelationElement dxUnitToUnitRelationElement, DXUnitDefinitionUnit dxUnitToAssign)
        {
            ...
            var relationType = dxUnit.DXUnitToUnitRelationElement.Announced.Single(x =>
                x.TargetDXUnit == dxUnitToUnitRelationElement.TargetDXUnit
                && x.TargetRelationName == dxUnitToUnitRelationElement.TargetRelationName
                ).RelationType;
        ...
        private DXUnitToUnitRelationElement GetRelationToDerivedDXUnitType(DXUnitDefinitionUnit dxUnit)
        {
            return new DXUnitToUnitRelationElement()
            {
                ...
                TargetDXUnit = Constants.DXUnitDefinitionUnitID,
                TargetRelationName = Constants.DerivedDXUnitType,
