    ...
    public abstract class DXTestFixtureBase : IAsyncLifetime, IDisposable
    {
        ...
        public DXTestFixtureBase()
        {
            IConfiguration configuration = new ConfigurationBuilder()
                ...
                {
                    { "Secrets:DatabaseConnectionString", $"Server=localhost;Database={Database};User Id=postgres;password=root;" },
            ...
            configuration["Secrets:DatabaseConnectionString"] = ReplaceDatabase(configuration["Secrets:DatabaseConnectionString"]!, Database);
            var connectionString = configuration["Secrets:DatabaseConnectionString"]!;
            ...
            services.AddSingleton<IDXTestSchemaHelper>(sp =>
            {
                ...
                var dbType = secrets.DatabaseType ?? "PostgreSQL";

                return dbType switch
                {
                    "PostgreSQL" => new PGSQLTestSchemaHelper(secrets.DatabaseConnectionString),
                    _ => throw new NotSupportedException($"No test schema helper for database type '{dbType}'.")
                };
            });
            ...
            try
            {
                ...
                for (var attempt = 1; attempt <= maxAttempts; attempt++)
                {
                    try
                    {
                        Root.DropDXDatabaseAsync().Wait();
                        Root.StartDXAsync().Wait();
                        break;
                    }
        ...
        public Task DisposeAsync() => Root.DropDXDatabaseAsync();
