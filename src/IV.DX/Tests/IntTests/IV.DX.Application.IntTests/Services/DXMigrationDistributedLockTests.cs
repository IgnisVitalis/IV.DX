...
namespace IV.DX.Application.IntTests.Services
{
    ...
    {
        private const string DatabaseName = "IV.DX.Application.IntTests";
        ...
        public async Task MigrateCustomEmbeddedAsync_WhenLockIsHeld_ThrowsTimeout()
        {
            ...
            using var lockHolderRoot = BuildIsolatedRoot(lockTimeoutSeconds: 30, lockName);
            using var contenderRoot = BuildIsolatedRoot(lockTimeoutSeconds: 1, lockName);
        ...
        public async Task MigrateCustomEmbeddedAsync_WhenLockReleased_Completes()
        {
            var lockName = $"IV.DX.IntTests.MigrationLock.{Guid.NewGuid():N}";
            using var lockHolderRoot = BuildIsolatedRoot(lockTimeoutSeconds: 30, lockName);
            using var contenderRoot = BuildIsolatedRoot(lockTimeoutSeconds: 3, lockName);
        ...
        private static ServiceProvider BuildIsolatedRoot(int lockTimeoutSeconds, string lockName)
        {
